---
layout: page
title: Creating a copy of an Oracle database from backup using RMAN on the same server
description: Creating a copy of an Oracle database from backup using RMAN on the same server
keywords: Oracle Database, RMAN, Creating a copy, ASM
permalink: /database/backup-and-restore/rman/duplicate-instance/duplicate-database-from-backup/
---

# Creating a copy of an Oracle database from backup using RMAN on the same server
...
### Have 1 Oracle 12c server, installed as <a href="/database/installation/single/asm/linux/6.7/oracle/12.1/">here</a>

<br/>

### Creating a password file for the new instance COPY12
...
### Specify connection parameters for the new instance COPY12
...
### Start instance COPY12 in nomount mode
...
### Configuring Listener for new service COPY12
...
### Preparing RMAN script for creating a server copy
...
Log file size values to be specified in the script.
...
Data is set according to the report schema query;
Data file identifiers (1,3,4....) see in report schema;
...
Completed without errors (on the 3rd try).
