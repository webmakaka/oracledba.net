---
layout: page
title: Creating backups using RMAN (Recovery Manager)
description: Creating backups using RMAN (Recovery Manager)
keywords: Oracle Database, RMAN, creating backups
permalink: /database/backup-and-restore/rman/oracle-rman-backup/
---

# Creating backups using RMAN (Recovery Manager)

### Backups can be stored in backup sets (default) and image copies:

<ul>
    <li>backup set  - data is stored in a format understandable only to RMAN. A backup set consists of Backup pieces, each of which can be a copy of a data file or a copy of a control file, or a copy of archive logs. </li>
    <li>image copies - differ from copies created, for example, with the cp command, only in that information about them is recorded in the control file or recovery catalog.</li>
</ul>
...
Creates a backup as a backup set

Command:<br/>

    RMAN> BACKUP AS COPY DATABASE;

Creates a backup as image copies
...
Provides information about available backup sets

Command:

    RMAN> LIST COPY;

Provides information about available image copies

Show the full list of archived logs

    RMAN> list archivelog all;

You can back up a specific datafile.

    RMAN> backup datafile 6;

The number can be found in

    RMAN> report schema;

You can back up a specific tablespace.

    RMAN> backup tablespace USERS, TEMP;

You can also compress backups to save space

    RMAN> CONFIGURE DEVICE TYPE DISK BACKUP TYPE TO COMPRESSED BACKUPSET;
    RMAN> BACKUP AS COMPRESSED BACKUPSET DATAFILE 1;

<br/>
<h3>Backups can have the following status:</h3>

<ul>
	<li>EXPIRED - RMAN marks backups and data copies as expired if, when running CROSSCHECK, references to missing or inaccessible files are found.</li>
	<li>OBSOLETE - a backup is considered obsolete if it is no longer needed for database recovery according to the current retention policy.</li>
</ul>
...
<br/>
<h3>Get information about files that need backup</h3>

    RMAN> REPORT NEED BACKUP;

<br/>
<h3>Create a backup, explicitly specifying the backup location:</h3>

    RMAN> BACKUP AS BACKUPSET DATABASE FORMAT '/tmp/%U';
    RMAN> BACKUP AS COPY DATABASE FORMAT '/tmp/%U';

<br/>
<h3>Create a backup of archived logs:</h3>

Archive logs can be either included in the backup or not.
<br/>
You can perform separate backup of archive logs.
<br/>
...
TAG "ARCHIVELOG_BACKUP" - defines the name for the created archivelog backup as "ARCHIVELOG_BACKUP".
<br/>

With time intervals specified

    RMAN> backup archivelog from time 'SYSDATE -1' DELETE ALL INPUT;

// I'm not sure what the parameters at the end mean
...
You can quickly find a backup by name.
<br/>
<br/>
...
### Create a copy of the current CONTROLFILE

    RMAN> BACKUP CURRENT CONTROLFILE TAG "CONTROLFILE";

<br/>

### Create a copy of SPFILE
...
Full backup (FULL BACKUP) - includes all data files, control file, and server parameter file (spfile).
...
Key - Unique identification key. <br/>
TY - Backup type: backup set (B) or copy (P).<br/>
...
Get information about the created backup.
...
### Get data on backup command results:
