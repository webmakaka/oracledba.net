---
layout: page
title: Creating Snapshot StandBy (Report Server) on Windows
description: Creating Snapshot StandBy (Report Server) on Windows
...
# Creating Snapshot StandBy (Report Server)
...
Victor (Telegram @Vict0rGI)

<br/>

One of the variants (Flashback) allows storing all changes during the day on the standby server in a temporary flash (size!!!) and at night roll forward to the current state, using only archive logs.
...
Change the archive log path, pointing to the existing StandBy log location:
...
After log recovery, switch to Snapshot mode, specify archive file path
...
Switch to archive log apply mode:
...
### Scripts for Snapshot StandBy operation

Run from local admin with ora_dba rights

// activation of physical StandBy, database recovery using latest archive logs up to a point in time  
**to_PhStBy.bat**
...
// activation of SnapShot StandBy with FlashBack  
**to_PhStBy.bat**
...
During a non-regular backup of the primary database, it is necessary to copy the archive log for subsequent recovery if needed (with a different SID): (RestTest10Arch.sql script)
...
// at the backup completion time (apply).  
**t.bat:**
