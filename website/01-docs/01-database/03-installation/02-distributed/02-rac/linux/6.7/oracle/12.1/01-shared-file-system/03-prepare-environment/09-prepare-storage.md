---
layout: page
title: Oracle RAC 12.1 SHARED FILE SYSTEM - Preparing storage server
description: Oracle RAC 12.1 SHARED FILE SYSTEM - Preparing storage server
...
# [Oracle RAC 12.1 SHARED FILE SYSTEM Installation]: Preparing storage server (RAID, NFS)

In real life everything should be done with hardware.
Although I could have initially created one large disk.

In fact. We have a group of disks that we want to combine into 1 disk and share it on the cluster nodes.
...
Creating a file system on top of the RAID array
...
Creating mdadm.conf configuration file

    # mkdir /etc/mdadm
    # echo "DEVICE partitions" > /etc/mdadm/mdadm.conf
    # mdadm --detail --scan --verbose | awk '/ARRAY/ {print}' >> /etc/mdadm/mdadm.conf

Creating a directory where RAID will be mounted locally
