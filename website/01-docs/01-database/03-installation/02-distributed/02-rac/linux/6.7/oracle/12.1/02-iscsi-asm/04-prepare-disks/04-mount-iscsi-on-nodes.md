---
layout: page
title: Oracle RAC 12.1 Installation on Oracle Linux 6.7 (ISCSI + ASM) - Mounting SCSI disks on cluster nodes
description: Oracle RAC 12.1 Installation on Oracle Linux 6.7 (ISCSI + ASM) - Mounting SCSI disks on cluster nodes
...
# [Oracle RAC 12.1 Installation on Oracle Linux 6.7 (ISCSI + ASM)]: Mounting SCSI disks on cluster nodes
...
Get the list of exported disks from storage:
...
### Configuring iSCSI disk mounting rules for cluster nodes.

In general, if not configured, they may connect arbitrarily, and as a result, data loss cannot be ruled out.
...
Get identifiers starting from /dev/sdc:

    # ls /dev/sd*
    /dev/sda   /dev/sda2  /dev/sdb1  /dev/sdd  /dev/sdf  /dev/sdh
    /dev/sda1  /dev/sdb   /dev/sdc   /dev/sde  /dev/sdg  /dev/sdi

On the virtual machine, virtual disks output the following results:
